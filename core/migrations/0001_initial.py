# Generated by Django 4.2.3 on 2023-09-15 14:54

import datetime
from django.conf import settings
import django.core.validators
from django.db import migrations, models
import django.db.models.deletion
import django_countries.fields


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='Category',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(max_length=100)),
                ('title_en', models.CharField(max_length=100)),
                ('title_ar', models.CharField(max_length=100)),
                ('slug', models.SlugField(max_length=190, unique=True)),
                ('image', models.ImageField(upload_to='')),
                ('is_active', models.BooleanField(default=True)),
            ],
            options={
                'verbose_name': 'Ajouter un categorie relie à des produit',
                'verbose_name_plural': 'Ajouter des categories relie à des produit',
            },
        ),
        migrations.CreateModel(
            name='Coupon',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('code', models.CharField(max_length=15)),
                ('amount', models.FloatField()),
            ],
            options={
                'verbose_name': 'Ajouter Un coupon pour un produit',
                'verbose_name_plural': 'Ajouter Un coupon pour un produit',
            },
        ),
        migrations.CreateModel(
            name='Essential',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('model_type', models.CharField(max_length=150)),
                ('model_type_en', models.CharField(max_length=150)),
                ('model_type_ar', models.CharField(max_length=150)),
                ('line1', models.CharField(max_length=100)),
                ('line1_en', models.CharField(max_length=100)),
                ('line1_ar', models.CharField(max_length=100)),
                ('line2', models.CharField(max_length=100)),
                ('line2_en', models.CharField(max_length=100)),
                ('line2_ar', models.CharField(max_length=100)),
                ('price', models.FloatField()),
                ('button_text', models.CharField(max_length=100)),
                ('button_text_en', models.CharField(max_length=100)),
                ('button_text_ar', models.CharField(max_length=100)),
                ('link', models.CharField(max_length=100)),
                ('image', models.ImageField(help_text='Size: 1920x570', upload_to='')),
                ('is_active', models.BooleanField(default=True)),
            ],
            options={
                'verbose_name': 'Ajouter un banner au centre de la page Home',
                'verbose_name_plural': 'Ajouter des banners au centre de la page Home',
            },
        ),
        migrations.CreateModel(
            name='ExcelFile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=100)),
                ('label', models.CharField(choices=[('A', 'ajouter'), ('M', 'modifier')], max_length=1)),
                ('file', models.FileField(upload_to='excel_files/')),
                ('gender', models.CharField(choices=[('M', 'MALE'), ('F', 'FEMALE'), ('M-F', 'MALE-FEMALE'), ('E', 'ENFANTS')], default='M', max_length=3)),
            ],
            options={
                'verbose_name': "Ajouter des Produits à partir d'un fichier excel",
                'verbose_name_plural': "Ajouter des Produits à partir d'un fichier excel",
            },
        ),
        migrations.CreateModel(
            name='Faq',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('question', models.CharField(max_length=500)),
                ('answer', models.CharField(max_length=500)),
            ],
            options={
                'verbose_name': 'Question & Answer',
                'verbose_name_plural': 'Questions & Answers',
            },
        ),
        migrations.CreateModel(
            name='Item',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('id_item', models.CharField(default='0', max_length=50)),
                ('title', models.CharField(max_length=100)),
                ('title_ar', models.CharField(max_length=100)),
                ('title_en', models.CharField(max_length=100)),
                ('price', models.FloatField()),
                ('brand_name', models.CharField(default='', max_length=150)),
                ('discount_price', models.FloatField(blank=True, null=True)),
                ('label', models.CharField(choices=[('S', 'Meilleur vente'), ('N', 'Nouveau'), ('P', 'Promotion')], max_length=1)),
                ('slug', models.SlugField(max_length=190, unique=True)),
                ('article_id', models.CharField(max_length=50)),
                ('stock_no', models.CharField(max_length=10)),
                ('description_short', models.CharField(max_length=5000)),
                ('description_long', models.TextField()),
                ('description_long_en', models.TextField()),
                ('description_long_ar', models.TextField()),
                ('details', models.CharField(default="{'color':''}", max_length=5500)),
                ('details_en', models.CharField(default="{'color':''}", max_length=5500)),
                ('details_ar', models.CharField(default="{'color':''}", max_length=5500)),
                ('tags', models.TextField()),
                ('rating', models.FloatField(blank=True, default=0.0, null=True, validators=[django.core.validators.MaxValueValidator(limit_value=5.0)])),
                ('gender', models.CharField(choices=[('M', 'MALE'), ('F', 'FEMALE'), ('M-F', 'MALE-FEMALE'), ('E', 'ENFANTS')], default='M', max_length=3)),
                ('color_exist', models.CharField(blank=True, default='', max_length=250, null=True)),
                ('color_not_exist', models.CharField(blank=True, default='', max_length=250, null=True)),
                ('size_exist', models.CharField(blank=True, default='34,36,38,40,42', max_length=250, null=True)),
                ('size_not_exist', models.CharField(blank=True, default='', max_length=250, null=True)),
                ('wishlist_num', models.FloatField(blank=True, default=0.0, null=True)),
                ('opinion_num', models.FloatField(blank=True, default=0.0, null=True)),
                ('image', models.ImageField(upload_to='')),
                ('shipping', models.CharField(max_length=250)),
                ('has_coupon', models.BooleanField(default=False)),
                ('coupon_code', models.CharField(blank=True, max_length=10, null=True)),
                ('price_after_coupon', models.FloatField(blank=True, null=True)),
                ('show_coupon', models.BooleanField(default=False)),
                ('coupon_start_date', models.DateField(blank=True, null=True)),
                ('coupon_end_date', models.DateField(blank=True, null=True)),
                ('is_active', models.BooleanField(default=True)),
                ('date_added', models.DateTimeField(default=datetime.datetime(2023, 9, 15, 14, 54, 27, 119137))),
                ('category', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.category')),
            ],
            options={
                'verbose_name': 'Ajouter Produit',
                'verbose_name_plural': 'Ajouter Produits',
            },
        ),
        migrations.CreateModel(
            name='Matacor_info',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('email', models.EmailField(max_length=254)),
                ('phone_number', models.CharField(max_length=50)),
                ('ccp', models.CharField(max_length=50)),
                ('name_owner', models.CharField(max_length=50)),
            ],
            options={
                'verbose_name': 'Matacor information ',
                'verbose_name_plural': 'Matacor informations (ccp/email,tel...)',
            },
        ),
        migrations.CreateModel(
            name='NewsLetterEmails',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('email', models.EmailField(max_length=254)),
            ],
            options={
                'verbose_name': 'NewsLetter Email',
                'verbose_name_plural': 'NewsLetter Emails',
            },
        ),
        migrations.CreateModel(
            name='Order',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ref_code', models.CharField(max_length=20)),
                ('start_date', models.DateTimeField(auto_now_add=True)),
                ('ordered_date', models.DateTimeField()),
                ('ordered', models.BooleanField(default=False)),
                ('total_amount', models.FloatField(default=1)),
                ('recui_image', models.ImageField(blank=True, default='', null=True, upload_to='orders_recu/')),
                ('phone_number', models.CharField(default='', max_length=10)),
                ('shipping_address', models.CharField(default='', max_length=50)),
                ('shipping_type', models.CharField(default='', max_length=50)),
                ('wilaya_ship', models.CharField(default='', max_length=50)),
                ('commun_ship', models.CharField(default='', max_length=50)),
                ('shipping_price', models.CharField(default='', max_length=10)),
                ('being_delivered', models.BooleanField(default=False)),
                ('received', models.BooleanField(default=False)),
                ('refund_requested', models.BooleanField(default=False)),
                ('refund_granted', models.BooleanField(default=False)),
                ('coupon', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to='core.coupon')),
            ],
            options={
                'verbose_name': 'la Commande confirmée',
                'verbose_name_plural': 'les Commandes confirmées',
            },
        ),
        migrations.CreateModel(
            name='ShopHeader',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('shop_header_image', models.ImageField(upload_to='shop_imgs')),
                ('main_title', models.CharField(max_length=50)),
                ('second_title', models.CharField(max_length=50)),
            ],
        ),
        migrations.CreateModel(
            name='Slide',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('caption1', models.CharField(max_length=100)),
                ('caption1_en', models.CharField(max_length=100)),
                ('caption1_ar', models.CharField(max_length=100)),
                ('caption2', models.CharField(max_length=100)),
                ('caption2_en', models.CharField(max_length=100)),
                ('caption2_ar', models.CharField(max_length=100)),
                ('link', models.CharField(max_length=100)),
                ('image', models.ImageField(help_text='Size: 1920x570', upload_to='')),
                ('is_active', models.BooleanField(default=True)),
            ],
            options={
                'verbose_name': 'Ajouter un Slide avec text et lien',
                'verbose_name_plural': 'Ajouter des Slides avec texts et liens',
            },
        ),
        migrations.CreateModel(
            name='WishList',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('items', models.ManyToManyField(to='core.item')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'utilisateur avec sa wishlists',
                'verbose_name_plural': 'utilisateurs avec leurs wishlists',
            },
        ),
        migrations.CreateModel(
            name='TopCategory',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(max_length=20)),
                ('slug', models.CharField(max_length=190)),
                ('items', models.ManyToManyField(to='core.category')),
            ],
            options={
                'verbose_name': 'Ajouter un category ex(Vettements,collections..)',
                'verbose_name_plural': 'Ajouter un category ex(Vettements,collections..)',
            },
        ),
        migrations.CreateModel(
            name='Refund',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('reason', models.TextField()),
                ('accepted', models.BooleanField(default=False)),
                ('email', models.EmailField(max_length=254)),
                ('order', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.order')),
            ],
        ),
        migrations.CreateModel(
            name='Profile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('image', models.ImageField(default='/static/images/default-avatar.jpg', upload_to='user_profiles')),
                ('fullname', models.CharField(default='', max_length=150)),
                ('email', models.EmailField(default='', max_length=250)),
                ('adresse', models.CharField(default='', max_length=250)),
                ('phone', models.CharField(default='', max_length=20)),
                ('user', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.CreateModel(
            name='Payment',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('stripe_charge_id', models.CharField(max_length=50)),
                ('amount', models.FloatField()),
                ('timestamp', models.DateTimeField(auto_now_add=True)),
                ('user', models.ForeignKey(blank=True, null=True, on_delete=django.db.models.deletion.SET_NULL, to=settings.AUTH_USER_MODEL)),
            ],
        ),
        migrations.CreateModel(
            name='OrderItem',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('ordered', models.BooleanField(default=False)),
                ('price_per_item', models.FloatField(blank=True, default=7.0, max_length=200, null=True)),
                ('size', models.CharField(default='', max_length=5)),
                ('quantity', models.IntegerField(default=1)),
                ('item', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.item')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'la Commande dans un panier',
                'verbose_name_plural': 'les Commandes dans les paniers non confirmée',
            },
        ),
        migrations.AddField(
            model_name='order',
            name='items',
            field=models.ManyToManyField(to='core.orderitem'),
        ),
        migrations.AddField(
            model_name='order',
            name='user',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL),
        ),
        migrations.CreateModel(
            name='Images_upload',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('images', models.ImageField(upload_to='images_upload_product/')),
                ('excel_related', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.excelfile')),
            ],
            options={
                'verbose_name': "pour l'upload des mutiples images visite /images/upload/",
                'verbose_name_plural': "pour l'upload des mutiples images visite /images/upload/",
            },
        ),
        migrations.CreateModel(
            name='ImageItem',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('slug', models.SlugField(max_length=190)),
                ('image', models.ImageField(upload_to='products/')),
                ('item', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.item')),
            ],
            options={
                'verbose_name': 'Ajouter Image de Produit',
                'verbose_name_plural': 'Ajouter Images des Produits',
            },
        ),
        migrations.CreateModel(
            name='GenderCategory',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('title', models.CharField(max_length=20)),
                ('slug', models.CharField(max_length=190)),
                ('categories', models.ManyToManyField(to='core.topcategory')),
            ],
            options={
                'verbose_name': 'Ajouter Gender category ex(hommes,femmes..)',
                'verbose_name_plural': 'Ajouter Gender category ex(hommes,femmes..)',
            },
        ),
        migrations.CreateModel(
            name='Comments_and_Ratings',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('image', models.ImageField(blank=True, default='', upload_to='images_comments')),
                ('rating', models.FloatField(max_length=1)),
                ('comment', models.TextField(max_length=500)),
                ('date_added', models.DateTimeField(default=datetime.datetime(2023, 9, 15, 14, 54, 27, 130729))),
                ('product', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.item')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'commentaire de chaque produit',
                'verbose_name_plural': 'commentaires de chaque produit',
            },
        ),
        migrations.CreateModel(
            name='Comment_images',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('image', models.ImageField(default='', upload_to='images_comments')),
                ('comment', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.comments_and_ratings')),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name': 'image de commentaire',
                'verbose_name_plural': 'les images des commentaires',
            },
        ),
        migrations.CreateModel(
            name='BillingAddress',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('street_address', models.CharField(max_length=100)),
                ('apartment_address', models.CharField(max_length=100)),
                ('country', django_countries.fields.CountryField(max_length=2)),
                ('zip', models.CharField(max_length=100)),
                ('address_type', models.CharField(choices=[('B', 'Billing'), ('S', 'Shipping')], max_length=1)),
                ('default', models.BooleanField(default=False)),
                ('user', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to=settings.AUTH_USER_MODEL)),
            ],
            options={
                'verbose_name_plural': 'BillingAddresses',
            },
        ),
        migrations.CreateModel(
            name='Banner_category',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('banner_image', models.ImageField(upload_to='banner_imgs')),
                ('banner_link', models.CharField(max_length=500)),
                ('category', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='core.topcategory')),
            ],
            options={
                'verbose_name': 'Ajouter un banner image pour un category ex(vettements, collection)',
                'verbose_name_plural': 'Ajouter un banner image pour un category  ex(vettements, collection)',
            },
        ),
    ]
